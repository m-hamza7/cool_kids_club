...
export default function Blog() {
  ...
  return (
    <div>
      ...
      <section className="pt-32 pb-16 px-6 text-center bg-[#FEF9EA]">
        <Animate animation="fade-up">
            <span className="text-[#9A7820] text-sm font-semibold uppercase tracking-widest">Knowledge</span>
      ...
      <section className="py-16 bg-white">
        <div className="max-w-6xl mx-auto px-6">
          <div className="flex items-center gap-2 mb-8">
            <Star className="w-5 h-5 text-[#D4A830]" />
            <h2 className="text-xl font-bold text-[#2d2d2d]">Featured This Month</h2>
          </div>
          <div className="grid md:grid-cols-2 gap-6">
            {featured.map((post, i) => (
              <Animate key={post.title} animation="fade-up" delay={i * 100}>
              <div className="rounded-2xl overflow-hidden border border-[#f0e9dd] hover:shadow-lg transition-shadow cursor-pointer group h-full">
                ...
                <div className="p-6">
                  <span className="px-2 py-1 rounded-full bg-[#EEF7EE] text-[#3D7840] text-xs font-semibold">
                    {post.category}
                  </span>
      ...
      {/* Filter */}
      <div className="bg-[#FAFAF5] border-y border-[#f0e9dd] px-6 py-4">
          ...
          {categories.map((cat) => (
            <button
              ...
              className={`px-4 py-2 rounded-full text-sm font-medium whitespace-nowrap transition-colors ${
                active === cat
                  ? 'bg-[#5DA05A] text-white'
                  : 'bg-white text-[#555] hover:bg-[#EEF7EE] border border-[#f0e9dd]'
              }`}
            >
              {cat}
            </button>
      ...
      <section className="py-16 bg-[#FAFAF5]">
        <div className="max-w-6xl mx-auto px-6">
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filtered.map((post, i) => (
              <Animate key={post.title} animation="fade-up" delay={i * 70}>
              <div className="bg-white rounded-2xl overflow-hidden border border-[#f0e9dd] hover:shadow-md transition-shadow cursor-pointer group h-full">
                <div className="overflow-hidden">
                  <img
                    src={post.img}
                    alt={post.title}
                ...
                <div className="p-5">
                  <span className="px-2 py-0.5 rounded-full bg-[#FEF9EA] text-[#9A7820] text-xs font-semibold">
                    {post.category}
                  </span>
      ...
      <section className="py-20 bg-white">
        <div className="max-w-6xl mx-auto px-6">
          <div className="text-center mb-12">
            <span className="text-[#9A7820] text-sm font-semibold uppercase tracking-widest">Free Downloads</span>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
            {resources.map((r, i) => (
              <Animate key={r.title} animation="fade-up" delay={i * 60}>
              <div
                className="flex items-start gap-4 bg-[#FAFAF5] rounded-2xl p-5 border border-[#f0e9dd] hover:shadow-md transition-shadow cursor-pointer h-full"
              >
                <div className={`w-11 h-11 rounded-xl ${r.bg} flex items-center justify-center flex-shrink-0 ${r.iconColor}`}>
                  {r.icon}
                </div>
                <div>
                  <div className="text-xs text-[#9A7820] font-semibold uppercase tracking-wide mb-1">{r.tag}</div>
